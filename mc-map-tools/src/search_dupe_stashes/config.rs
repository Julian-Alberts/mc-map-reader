    ...
    required_nbt.iter().all(|(required_key, required_value)| {
        ...
        match (required_value, item_value) {
            ...
            (JsonValue::Number(required_value), Some(NbtValue::Byte(item_value))) => {
                required_value.is_i64() && required_value.as_i64().expect("Error converting number") as i8 == *item_value
            },
            (JsonValue::Number(required_value), Some(NbtValue::Double(item_value))) => {
                required_value.is_f64() && required_value.as_f64().expect("Error converting number") as f64 == *item_value
            },
            (JsonValue::Number(required_value), Some(NbtValue::Float(item_value))) => {
                required_value.is_f64() && required_value.as_f64().expect("Error converting number") as f32 == *item_value
            },
            (JsonValue::Number(required_value), Some(NbtValue::Int(item_value))) => {
                required_value.is_i64() && required_value.as_i64().expect("Error converting number") as i32 == *item_value
            },
            (JsonValue::Number(required_value), Some(NbtValue::Long(item_value))) => {
                required_value.is_i64() && required_value.as_i64().expect("Error converting number") as i64 == *item_value
            },
            (JsonValue::Number(required_value), Some(NbtValue::Short(item_value))) => {
                required_value.is_i64() && required_value.as_i64().expect("Error converting number") as i16 == *item_value
            },
            (JsonValue::Object(required_value), Some(NbtValue::Compound(item_value))) => {
                filter_nbt_eq_to_item_nbt(required_value, item_value)
            },
            (JsonValue::String(required_value), Some(NbtValue::String(item_value))) => required_value == item_value,
